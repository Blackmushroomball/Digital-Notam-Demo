...
public final class BaselineAirportHeliportCatalog {
    ...
    public BaselineAirportHeliportCatalog(){try(InputStream in=Files.newInputStream(Path.of("config","donlon-baseline.properties"))){config.load(in);}catch(Exception e){throw new IllegalStateException("无法读取机场基线配置: "+e.getMessage(),e);}}

    public List<AirportSummary> list()throws Exception{List<AirportSummary> result=new ArrayList<>();for(String code:required("airports").split(",")){AirportBaseline b=findLatest(code.trim());result.add(new AirportSummary(b.designator(),b.name(),b.type(),b.locationIndicatorIcao(),b.latitude(),b.longitude(),b.firDesignator(),b.firSource()));}return result;}
    public String json()throws Exception{return "["+list().stream().map(a->"{\"designator\":\""+a.designator()+"\",\"name\":\""+a.name()+"\",\"type\":\""+a.type()+"\",\"locationIndicatorICAO\":\""+a.locationIndicatorIcao()+"\",\"latitude\":\""+a.latitude()+"\",\"longitude\":\""+a.longitude()+"\",\"fir\":\""+a.firDesignator()+"\",\"firSource\":\""+a.firSource()+"\"}").reduce((a,b)->a+","+b).orElse("")+"]";}
    public AirportBaseline find(String designator,Instant start,Instant end)throws Exception{return read(designator,start,end);}
    ...
    public static List<Element> directChildren(Element parent,String local){List<Element> r=new ArrayList<>();for(Node n=parent.getFirstChild();n!=null;n=n.getNextSibling())if(n instanceof Element e&&local.equals(e.getLocalName()))r.add(e);return r;}
    public static Element first(Element parent,String local){NodeList n=parent.getElementsByTagNameNS("*",local);return n.getLength()==0?null:(Element)n.item(0);}
    public static String text(Element parent,String local){Element e=first(parent,local);return e==null?"":e.getTextContent().trim();}
    public record AirportBaseline(String designator,String name,String type,String locationIndicatorIcao,String uuid,String latitude,String longitude,String firDesignator,String firUuid,String firSource,List<Element> normalAvailabilities){}
    public record AirportSummary(String designator,String name,String type,String locationIndicatorIcao,String latitude,String longitude,String firDesignator,String firSource){}
