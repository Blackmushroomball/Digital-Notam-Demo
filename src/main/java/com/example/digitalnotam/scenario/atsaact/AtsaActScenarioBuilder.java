...
public final class AtsaActScenarioBuilder implements ScenarioBuilder {
    ...
    private static void addSchedule(Document d,Element a,Notam n,boolean excluded){List<String> days="WEEKDAYS".equals(n.scheduleMode())?csv(n.scheduleDay()):List.of("ANY");Node before=null;for(Node x=a.getFirstChild();x!=null;x=x.getNextSibling())if(x instanceof Element e&&Set.of("annotation","specialDateAuthority","activity","status").contains(e.getLocalName())){before=x;break;}for(String day:days){Element ti=aixm(d,"timeInterval"),ts=id(d,"Timesheet");ti.appendChild(ts);add(d,ts,"timeReference","UTC");if("DATES".equals(n.scheduleMode())){add(d,ts,"startDate",date(n.scheduleStartDate()));add(d,ts,"endDate",date(n.scheduleEndDate()));}add(d,ts,"day",day);add(d,ts,"startTime",n.scheduleStart());add(d,ts,"endTime",n.scheduleEnd());add(d,ts,"daylightSavingAdjust","NO");add(d,ts,"excluded",excluded?"YES":"NO");a.insertBefore(ti,before);}}
    private static void annotation(Document d,Element a,String value){Element w=children(a,"annotation").stream().findFirst().orElse(null);if(w==null){w=aixm(d,"annotation");Node before=null;for(Node n=a.getFirstChild();n!=null;n=n.getNextSibling())if(n instanceof Element e&&Set.of("specialDateAuthority","activity","status").contains(e.getLocalName())){before=n;break;}a.insertBefore(w,before);}else{while(w.hasAttributes())w.removeAttributeNode((Attr)w.getAttributes().item(0));}Element note=id(d,"Note");w.appendChild(note);add(d,note,"purpose","REMARK");Element tr=aixm(d,"translatedNote"),ln=id(d,"LinguisticNote"),text=add(d,ln,"note",value);text.setAttribute("lang","ENG");tr.appendChild(ln);note.appendChild(tr);}
