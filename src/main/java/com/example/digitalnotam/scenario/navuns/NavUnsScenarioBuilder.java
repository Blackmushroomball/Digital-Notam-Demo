...
public final class NavUnsScenarioBuilder implements ScenarioBuilder {
    ...
    private static void validateSchedule(Notam n){EventScheduleSupport.validate(n);}
    ...
    private static void addSchedule(Document d,Element status,Notam n,boolean excluded){Node before=direct(status,"annotation");if(before==null)before=direct(status,"specialDateAuthority");EventScheduleSupport.append(d,status,before,n,excluded);}
